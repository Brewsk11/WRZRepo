...
DWORD WINAPI WatekOdbioru(void *ptr)
{
	...
	while (1)
	{
		...
		case WSPOLPRACA:
		{
			//if (ramka.iID != my_vehicle->iID)  // ID pojazdu, ktory otrzymal przelew zgadza siê z moim ID 
			//{
			std::stringstream ss;
			ss << "Gracz o id=" << ramka.iID << " zaprasza do zespolu!";
			string asd = ss.str();
		...
		case WSPOLPRACA_OK:
		{
			MovableObject* mo = network_vehicles[ramka.iID];
			if (mo != nullptr) 
			{
				my_vehicle->ID_koalicjanta = ramka.iID;
				my_vehicle->OwnObjectColorCooperation[0] = mo->OwnObjectColorCooperation[0];
...
void PoczatekInterakcji()
{
	...
	multi_reciv = new multicast_net("224.10.12.67", 10001);      // obiekt do odbioru ramek sieciowych
	multi_send = new multicast_net("224.10.12.67", 10001);       // obiekt do wysy³ania ramek
...
void WyslanieProsbyOWspolprace()
{
	Ramka ramka;
	...
	int       nCmdShow)
{
	...
	okno = CreateWindow(nazwa_klasy, "WZR 2018/19, temat 3, wersja b ", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
...
void KlawiszologiaSterowania(UINT kod_meldunku, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case 'P':  // rozpoczecie interakcji zespolowej
		{
		...
		case 'O': //wpisanie id kooperacji
		{
			...
			SetWindowText(okno, "Wpisz id gracza do wspolpracy.");
			break;
		}
		...
		default: {
			...
			if (wParam >= 96 && wParam <= 106 && cooperationFlag) {
				if (cooperationId >= 1000)
					cooperationId = 0;
				int digit = wParam - 96;
				cooperationId = cooperationId * 10 + digit;

				std::stringstream ss;
				ss << " Wpisz id gracza: " << cooperationId;
				string napis = ss.str();
				SetWindowText(okno, napis.c_str());
			}
			break;
		}
