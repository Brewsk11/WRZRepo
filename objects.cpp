...
MovableObject::MovableObject(Terrain *t)             // konstruktor                   
{
	...
	//ustalenie losowych kolorow druzyn
	std::random_device rd;
	std::mt19937 gen(rd()); 
	std::uniform_real_distribution<> dis(0.0, 2.0);
	
	OwnObjectColor[0] = dis(gen);
	OwnObjectColor[1] = dis(gen);
	OwnObjectColor[2] = dis(gen);
	OwnObjectColor[3] = 1.0f;

	OwnObjectColorCooperation[0] = dis(gen);
	OwnObjectColorCooperation[1] = dis(gen);
	OwnObjectColorCooperation[2] = dis(gen);
	OwnObjectColorCooperation[3] = dis(gen);
...
bool MovableObject::IsInCooperation()
{
	if (this->ID_koalicjanta != -1)
		return true;
	else return false;
}
...
void Terrain::Rysuj()
{
	...
	GLfloat GreenSurface[] = { 0.45f, 0.42f, 0.1f, 1.0f };
