...
void RysujScene()
{
	...
	GLfloat GroundSurface[] = { 0.35f, 0.42f, 0.1f, 1.0f };
	...
	for (int w = -dw; w < 1 + dw; w++)
		for (int k = -dk; k < 1 + dk; k++)
		{
			...
			if (my_vehicle->IsInCooperation())
				glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, my_vehicle->OwnObjectColorCooperation);
			else
				glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, my_vehicle->OwnObjectColor);
